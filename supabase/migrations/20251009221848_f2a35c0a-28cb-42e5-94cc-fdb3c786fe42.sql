...
-- 2) Fix the api_connections table RLS policy to allow proper SELECT
-- Remove the overly restrictive policy that blocks all SELECT
DROP POLICY IF EXISTS "Clients cannot directly select from api_connections" ON api_connections;

-- Create a proper policy that allows users to view their own connections
...
-- 4) Enable RLS on the view (Postgres will enforce base table's RLS)
